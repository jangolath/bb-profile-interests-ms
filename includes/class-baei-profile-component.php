<?php
/**
 * BAE Profile Component for Interests
 */
...
class BAEI_Profile_Component {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_bae_load_more_interests', array($this, 'ajax_load_more_interests'));
    }
    ...
    public function setup_profile_nav() {
        ...
        bp_core_new_nav_item(array(
            'name' => __('Interests', 'bae-interests'),
            'slug' => 'interests',
            'position' => $position,
    ...
    public function profile_template_title() {
        if (bp_is_my_profile()) {
            echo __('My Interests', 'bae-interests');
        } else {
            echo sprintf(__('%s\'s Interests', 'bae-interests'), bp_get_displayed_user_fullname());
        }
    }
    ...
    public function profile_template_content() {
        ...
        include BAEI_PLUGIN_DIR . 'templates/profile-interests.php';
    }
    ...
    public function profile_manage_template_title() {
        echo __('Manage My Interests', 'bae-interests');
    }
    ...
    public function profile_manage_template_content() {
        ...
        if (!bp_is_my_profile()) {
            ...
            echo '<p>' . __('You do not have permission to manage this user\'s interests.', 'bae-interests') . '</p>';
            echo '</div>';
            return;
        }
        ...
        include BAEI_PLUGIN_DIR . 'templates/manage-interests.php';
    }
    ...
    public function ajax_load_more_interests() {
        ...
        if (empty($_POST['category_id']) || !isset($_POST['offset'])) {
            wp_send_json_error(array('message' => __('Missing required parameters.', 'bae-interests')));
        }
        ...
        foreach ($interests as $interest) {
            ...
            $html .= '<div class="bae-interest-item">';
            $html .= '<a href="' . esc_url(home_url('/members/?interest=' . $interest->slug)) . '" class="bae-interest-link">';
            $html .= esc_html($interest->name);
            $html .= '</a>';
            
            if (bp_is_my_profile() || is_admin()) {
                $html .= '<div class="bae-interest-actions">';
                if ($has_interest) {
                    $html .= '<button class="bae-remove-interest" data-interest-id="' . esc_attr($interest->id) . '">';
                    $html .= '<span class="dashicons dashicons-no-alt"></span>';
                    $html .= '</button>';
                } else {
                    $html .= '<button class="bae-add-interest" data-interest-id="' . esc_attr($interest->id) . '">';
