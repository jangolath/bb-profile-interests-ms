...
class BB_Interests_Frontend {
    ...
    public function __construct() {
        // Add shortcodes
        add_shortcode('bb_interests_list', array($this, 'shortcode_interests_list'));
    ...
    public function register_widgets() {
        register_widget('BB_Interests_Widget');
    }
    ...
    public function shortcode_interests_list($atts) {
        ...
        if (empty($interests)) {
            echo '<p>' . __('No interests found.', 'bb-interests') . '</p>';
            return ob_get_clean();
        }
        
        echo '<div class="bb-interests-list bb-interests-columns-' . esc_attr($columns) . '">';
        
        foreach ($interests as $interest) {
            echo '<div class="bb-interest-item">';
            echo '<a href="' . esc_url(home_url('/members/?interest=' . $interest->slug)) . '" class="bb-interest-link">';
            echo esc_html($interest->name);
            
            if ($show_count) {
                $count = $db->count_users_by_interest($interest->id);
                echo ' <span class="bb-interest-count">(' . esc_html($count) . ')</span>';
            }
...
/**
 * BuddyBoss Interests Widget
 */
class BB_Interests_Widget extends WP_Widget {
    ...
    public function __construct() {
        parent::__construct(
            'bb_interests_widget',
            __('Interests', 'bb-interests'),
            array(
                'description' => __('Display a list of popular interests.', 'bb-interests'),
                'classname' => 'widget_bb_interests'
            )
        );
    }
    ...
    public function widget($args, $instance) {
        ...
        if (empty($interests)) {
            ...
            echo '<ul class="bb-interests-widget-list">';
            
            foreach ($interests as $interest) {
                echo '<li class="bb-interest-widget-item">';
                echo '<a href="' . esc_url(home_url('/members/?interest=' . $interest->slug)) . '" class="bb-interest-link">';
                echo esc_html($interest->name);
                
                if (!empty($instance['show_count'])) {
                    echo ' <span class="bb-interest-count">(' . esc_html($interest->user_count) . ')</span>';
                }
    ...
    public function form($instance) {
        $title = !empty($instance['title']) ? $instance['title'] : __('Popular Interests', 'bb-interests');
        $limit = !empty($instance['limit']) ? intval($instance['limit']) : 10;
        $show_count = isset($instance['show_count']) ? (bool) $instance['show_count'] : true;
        
        ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title:', 'bb-interests'); ?></label>
            <input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($title); ?>">
        </p>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('limit')); ?>"><?php esc_html_e('Number of interests to show:', 'bb-interests'); ?></label>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr($this->get_field_id('show_count')); ?>"><?php esc_html_e('Display user count', 'bb-interests'); ?></label>
        </p>
        <?php
    }
